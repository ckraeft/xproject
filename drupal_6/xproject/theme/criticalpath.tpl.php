                    ...
                    <? if(isset($taskinfo['subtasks'])): ?>
                        <table class="tasklist">
                            ...
                            <? foreach($taskinfo['subtasks'] as $subtaskinfo) { ?>
                                <td class="task department_<? echo $subtaskinfo['department']; ?>">
                                    ...
                                    <div class="taskDates"><strong><? if(strtotime($subtaskinfo['planned_start_date']) > 0) { $planned_start_date = explode(' ', $subtaskinfo['planned_start_date']); print date('y/m/d', strtotime($subtaskinfo['planned_start_date'])); } ?></strong> <em><? print $subtaskinfo['assigned_to_name']; ?></em></div>
